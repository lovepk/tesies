/*!
 * jquery.ua.js
 * @author 愚人码头 http://www.css88.com
 * @version 1.0
 */
...
(function ($, win, undefined) {
    ...
    // [10,)版本就无法判断
        ...
        uc_proxy = UA.match(/(UCWEB)(\d.+?(?=\/))/),
        qq = UA.match(/(MQQBrowser)\/(\d.+?(?=\/ |\sMozilla)).*AppleWebKit\/([\d.]+)/),
        wp7 = (UA.indexOf('WP7') > -1) || (UA.indexOf('Windows Phone OS') > -1);
    if (isIe6) {
        ieVer = 6;
    } else if (isIe7) {
        ieVer = 7;
    ...
    $.extend({
        ua: function () {
            ...
            return {
                ...
                isTaobao: _getChromiumType() === 'taobao',
                isOpera: _getChromiumType() === 'opera',
    ...
    function _getChromiumType() {
        ...
        var taobrowser=userAgent.indexOf('taobrowser') != -1;
        // 原始 chrome
        if (isOriginalChrome) {
            if(!!userAgent.match(/taobrowser.([\d.]+)/)){
                                return 'taobao'
                            }
            return 'chrome';
        }
